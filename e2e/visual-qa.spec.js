...
test('capture all 9 primary screens', async ({ page }) => {
  ...
  const publicGuestListToggle = page.getByRole('button', { name: /Popis gostiju po stolovima/ })
  await publicGuestListToggle.scrollIntoViewIfNeeded()
  await publicGuestListToggle.click()
  await settle(page)
  await page.screenshot({ path: `${outDir}/07b-public-table-list.png` })
