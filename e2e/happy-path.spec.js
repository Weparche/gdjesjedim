...
test('organizer happy path: create event, assign table, publish, guest finds table', async ({ page }) => {
  ...
  const scheduleTitle = page.getByRole('heading', { name: 'Raspored događaja' })
  ...
  await expect(page.getByRole('link', { name: 'Otvori Lido parking na karti' })).toBeVisible()
  expect((await galleryTitle.boundingBox()).y).toBeLessThan((await scheduleTitle.boundingBox()).y)
  expect((await scheduleTitle.boundingBox()).y).toBeLessThan((await mapTitle.boundingBox()).y)
